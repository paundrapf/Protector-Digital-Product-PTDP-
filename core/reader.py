...
class PTDPReader:
    ...
    def read_metadata(self, ptdp_file):
        """
        Baca metadata tanpa decrypt content
        
        Args:
            ptdp_file: Path ke .ptdp file
        
        Returns:
            dict: Metadata information
        """
        if not os.path.exists(ptdp_file):
            raise FileNotFoundError(f"File not found: {ptdp_file}")
        
        with open(ptdp_file, 'rb') as f:
            # Validate header
            magic = f.read(4)
            if magic != self.MAGIC_BYTES:
                raise ValueError("Invalid PTDP file format! This is not a valid .ptdp file.")
    ...
    def decrypt_file(self, ptdp_file, license_key, output_file=None, master_password=None):
        """
        Decrypt .ptdp file dengan validasi license
        ...
        """
        print(f"🔓 Opening protected file: {ptdp_file}")
        
        if not os.path.exists(ptdp_file):
            return {
                'success': False,
                'error': f'File not found: {ptdp_file}'
            }
        ...
        try:
            ...
            with open(ptdp_file, 'rb') as f:
                ...
                if magic != self.MAGIC_BYTES:
                    return {
                        'success': False,
                        'error': 'Invalid PTDP file format'
                    }
    ...
    def get_file_info(self, ptdp_file):
        """Get detailed information about .ptdp file"""
        try:
            metadata = self.read_metadata(ptdp_file)
            file_size = os.path.getsize(ptdp_file)
...
def decrypt_file_cli():
    ...
    print("🔓 PTDP File Decryption Tool")
    print("=" * 50)
    
    # Input file
    ptdp_file = input("PTDP file path: ").strip()
    if not os.path.exists(ptdp_file):
        print(f"❌ File not found: {ptdp_file}")
        return
    
    # Show metadata first
    reader = PTDPReader()
    try:
        metadata = reader.read_metadata(ptdp_file)
    ...
    # Output file
    default_output = ptdp_file.replace('.ptdp', metadata['original_extension'])
    ...
    # Decrypt
    print("\n" + "=" * 50)
    result = reader.decrypt_file(ptdp_file, license_key, output_file)
