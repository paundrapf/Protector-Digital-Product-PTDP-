#!/usr/bin/env python3
"""
Test Suite untuk PTDP Protection System
...
class TestProtection(unittest.TestCase):
    def setUp(self):
        ...
        self.test_file.write("This is test content for PTDP protection system.\n" * 100)
        self.test_file.close()
        
        self.ptdp_file = self.test_file.name.replace('.txt', '.ptdp')
        self.output_file = self.test_file.name.replace('.txt', '_decrypted.txt')
    
    def tearDown(self):
        """Cleanup test files"""
        for f in [self.test_file.name, self.ptdp_file, self.output_file]:
            if os.path.exists(f):
                os.remove(f)
    ...
    def test_file_protection(self):
        ...
        self.assertTrue(os.path.exists(self.ptdp_file))
    ...
    def test_file_decryption(self):
        ...
        result = reader.decrypt_file(
            ...
            license,
            self.output_file,
            master_password='TestPassword123'  # Must match creation password
        )
    ...
    def test_metadata_reading(self):
        ...
        # Read metadata
        reader = PTDPReader()
        metadata = reader.read_metadata(self.ptdp_file)
...
def run_tests():
    """Run all tests"""
    print("\n" + "="*60)
    print("PTDP Protection System - Test Suite")
    print("="*60 + "\n")
