# PTDP Protection System - Developer Guide
...
PTDP is a complete digital product protection system that includes:
...
### Verify Installation
```bash
python -c "from core import PTDPCreator; print('✅ Installation successful!')"
```
...
Buyers can find their Device ID in PTDP Viewer (bottom of window). They should send this to you.
...
```
1. CREATE PRODUCT
   ├─ Protect file with PTDP Creator
   ├─ Save master password securely
   └─ Upload .ptdp file to distribution platform

2. BUYER PURCHASES
   ├─ Buyer downloads .ptdp file
   ├─ Buyer downloads PTDP Viewer (if needed)
...
4. BUYER USES PRODUCT
   ├─ Opens .ptdp file in PTDP Viewer
   ├─ Enters license key
...
# Install Heroku CLI
heroku login
heroku create ptdp-license-server
...
class PTDPCreator:
    def create_protected_file(input_file, output_file, config):
        ...
        Args:
            ...
            output_file (str): Path to save .ptdp file
...
```python
class PTDPReader:
    def decrypt_file(ptdp_file, license_key, output_file=None):
        """
        Decrypt protected file
        
        Args:
            ptdp_file (str): Path to .ptdp file
...
This creates:
- `dist/PTDP-Viewer.exe` - For end-users
- `dist/PTDP-Creator.exe` - For you

### Distribute to Users

1. Package `PTDP-Viewer.exe` with:
   - `README.txt` (user guide)
   - `register_ptdp.bat` (optional)
...
© 2024 PTDP Protection System - Developer Guide
